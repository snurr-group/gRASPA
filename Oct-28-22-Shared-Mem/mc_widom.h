...
static inline double Widom_Move_FirstBead(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, bool Retrace, double &StoredR, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, bool DualPrecision)
{
  ...
  if(Widom.UseGPUReduction) //Perform Reduction on the GPU//
  {
    ...
    {
      GPU_Sum_Widom(NumberOfTrials, FF.Beta, OverlapCriteria, Widom.floatResult, Widom.flag, arraysize, energies, Trialindex, Rosen);
    }
    else
    {
      GPU_Sum_Widom(NumberOfTrials, FF.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, Widom.flag, arraysize, energies, Trialindex, Rosen);
    }
  }
  else //Perform Reduction on the CPU//
  {
    if(DualPrecision && NumberOfTrials > 1){ //If there is only 1 trial position, there is no need to do Dual-Precision CBMC//
      Host_sum_Widom(NumberOfTrials, FF.Beta, static_cast<float>(OverlapCriteria), Widom.floatResult, Widom.flag, arraysize, energies, Trialindex, Rosen);}
    else{
      Host_sum_Widom(NumberOfTrials, FF.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, Widom.flag, arraysize, energies, Trialindex, Rosen);}
  }
...
static inline double Widom_Move_Chain(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, size_t FirstBeadTrial, bool DualPrecision)
{
  ...
  {
    if(DualPrecision){
      GPU_Sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, OverlapCriteria, Widom.floatResult, Widom.flag, arraysize*chainsize, energies, Trialindex, Rosen);}
    else{
      GPU_Sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, Widom.flag, arraysize*chainsize, energies, Trialindex, Rosen);}
  }
  else //Sum on the CPU//
  {
    if(DualPrecision){
      Host_sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, static_cast<float>(OverlapCriteria), Widom.floatResult, Widom.flag, arraysize*chainsize, energies, Trialindex, Rosen);}
    else{
      Host_sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, Widom.flag, arraysize*chainsize, energies, Trialindex, Rosen);}
  }
  ...
  if(Goodconstruction)
  {
    ...
    if(DualPrecision)
    {
      Update_Dual_Precision_Rosenbluth(Widom, FF.Beta, arraysize*chainsize, SelectedTrial, REALselected, energies, averagedRosen);
    }
  }
  else
  {
...
static inline double Widom_Move_FirstBead_PARTIAL(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, bool Retrace, double &StoredR, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, bool DualPrecision)
{
  ...
  //     Reduction     //
  ///////////////////////
  Host_sum_Widom(NumberOfTrials, FF.Beta, OverlapCriteria, Widom.Blocksum, Widom.flag, Nblock, energies, Trialindex, Rosen);
  if(!Retrace && Rosen.size() == 0) return;
  for(size_t a = 0; a < Rosen.size(); a++) Rosen[a] = std::exp(Rosen[a]);
  Rosenbluth =std::accumulate(Rosen.begin(), Rosen.end(), decltype(Rosen)::value_type(0));
  if(!Retrace && Rosenbluth < 1e-150) return;
...
static inline double Widom_Move_Chain_PARTIAL(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, size_t FirstBeadTrial, bool DualPrecision)
{
  ...
  //    Reduction   //
  ////////////////////
  Host_sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, OverlapCriteria, Widom.Blocksum, Widom.flag, Nblock, energies, Trialindex, Rosen);
