...
static inline double Widom_Move_Chain(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, size_t FirstBeadTrial, bool DualPrecision)
{
  ...
  size_t flag[Widom.NumberWidomTrialsOrientations]; for(size_t i=0;i<Widom.NumberWidomTrialsOrientations;i++){flag[i]=0;}
  size_t* device_flag;
  ...
  {
    if(DualPrecision){
      GPU_Sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, OverlapCriteria, widomfloat, flag, arraysize*chainsize, energies, Trialindex, Rosen);}
    else{
      GPU_Sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, flag, arraysize*chainsize, energies, Trialindex, Rosen);}
  }
  else //Sum on the CPU//
  {
    if(DualPrecision){
      Host_sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, static_cast<float>(OverlapCriteria), widomfloat, flag, arraysize*chainsize, energies, Trialindex, Rosen);}
    else{
      Host_sum_Widom(Widom.NumberWidomTrialsOrientations, FF.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, flag, arraysize*chainsize, energies, Trialindex, Rosen);}
  }
