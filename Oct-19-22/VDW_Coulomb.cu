...
__global__ void Framework_energy_difference_SoA(Boxsize Box, Atoms* System, Atoms Mol, Atoms NewMol, ForceField FF, double* y, double* dUdlambda, size_t ComponentID, size_t totalthreads, size_t Molsize) // Consider to change this for polyatomic
{
  ///////////////////////////////////////////////////////
  //All variables passed here should be device pointers//
  ///////////////////////////////////////////////////////
  ...
  {
  for (size_t j = 0; j < Molsize; j++) //NEW POSITION
  {
  ...
  for (size_t j = 0; j < Molsize; j++) //OLD POSITION
  {
...
__global__ void Collapse_Framework_Energy_OVERLAP_FLOAT(Boxsize Box, Atoms* System, Atoms NewMol, ForceField FF, double* y, size_t ComponentID, size_t totalAtoms, size_t* flag, size_t totalthreads, size_t trialsize, float* y_float)
{
  ...
  if(!((MoleculeID == NewMol.MolID[0]) &&(comp == ComponentID)))
  {
    ...
    if(rr_dot < VDWCutoff)
    {
      ...
      const double FFarg[4] = {FF.epsilon[row], FF.sigma[row], FF.z[row], FF.shift[row]};
      VDW(FFarg, rr_dot, scaling, result); 
      if(result[0] > OverlapCriteria){ flag[k]++;}
      tempy += result[0];
    }
