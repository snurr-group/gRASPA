...
double Widom_Move_FirstBead_PARTIAL(Components& SystemComponents, Simulations& Sims, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, int MoveType, double &StoredR, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, double2 proposed_scale);
...
template<typename T>
inline void Host_sum_Widom_HGGG(size_t NumberWidomTrials, double Beta, T Overlap, T* energy_array, bool* flag, size_t HG_Nblock, size_t HGGG_Nblock, std::vector<MoveEnergy>& energies, std::vector<size_t>& Trialindex, std::vector<double>& Rosen)
{
  std::vector<size_t>reasonable_trials;
  for(size_t i = 0; i < NumberWidomTrials; i++){
    if(!flag[i]){
      reasonable_trials.push_back(i);}}
  T host_array[HGGG_Nblock];
  for(size_t i = 0; i < reasonable_trials.size(); i++)
  {
    size_t trial = reasonable_trials[i];
    cudaMemcpy(host_array, &energy_array[trial*HGGG_Nblock], HGGG_Nblock*sizeof(T), cudaMemcpyDeviceToHost);
    T HG_tot = 0.0;
    T GG_tot = 0.0;
    //Zhao's note: If during the pairwise interaction, there is no overlap, then don't check overlap in the summation//
    //Otherwise, it will cause issues when using the fractional molecule (energy drift in retrace)//
    //So translation/rotation summation of energy are not checking the overlaps, so this is the reason for the discrepancy//
    for(size_t ijk=0; ijk < HG_Nblock; ijk++)           HG_tot+=host_array[ijk];
    for(size_t ijk=HG_Nblock; ijk < HGGG_Nblock; ijk++) GG_tot+=host_array[ijk];
    
    double tot = static_cast<double>(HG_tot + GG_tot);
   
    MoveEnergy E;
    E.HGVDWReal = HG_tot; E.GGVDWReal = GG_tot;
...
static inline double Widom_Move_FirstBead_PARTIAL(Components& SystemComponents, Simulations& Sims, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, int MoveType, double &StoredR, size_t *REAL_Selected_Trial, bool *SuccessConstruction, MoveEnergy *energy, double2 proposed_scale)
{
  ...
  //Zhao's note: For CBMC_INSERTION, the chosen molecule ALWAYS EXISTS. so it is OK to set start_position to the start_position of ANY molecule//
  //Although rigorously speaking, for insertion it should always read the charge and type from the first molecule (start_position = 0)
  ...
  Random.Check(NumberOfTrials * 3);
  get_random_trial_position_firstbead_SWITCH<<<1,NumberOfTrials>>>(Sims.Box, Sims.d_a, Sims.New, Sims.device_flag, Random.device_random, Random.offset, start_position, SelectedComponent, SelectedMolID, MoveType, proposed_scale); checkCUDAError("error getting random trials");
  Random.Update(NumberOfTrials * 3);

  // Setup the pairwise calculation //
  size_t Nthread=0; size_t Nblock=0;
  ...
  if(Atomsize != 0)
  {
    //Setup_threadblock(Atomsize, &Nblock, &Nthread); //for partial reduction, Nblock is number of blocks for **each trial**//
    //printf("Atomsize: %zu, Nblock: %zu, Nthread: %zu\n", Atomsize, Nblock, Nthread);
    //Collapse_Framework_Energy_OVERLAP_PARTIAL<<<Nblock*NumberOfTrials, Nthread, Nthread * sizeof(double)>>>(Sims.Box, Sims.d_a, Sims.New, FF, Sims.Blocksum, SelectedComponent, Atomsize, Sims.device_flag, threadsNeeded,1, Nblock); checkCUDAError("Error calculating energies (PARTIAL SUM)");

    Collapse_Framework_Energy_OVERLAP_PARTIAL_HGGG<<<HGGG_Nblock * NumberOfTrials, HGGG_Nthread, HGGG_Nthread * sizeof(double)>>>(Sims.Box, Sims.d_a, Sims.New, FF, Sims.Blocksum, SelectedComponent, Atomsize, Sims.device_flag, threadsNeeded,1, HGGG_Nblock, HG_Nblock); checkCUDAError("Error calculating energies (PARTIAL SUM HGGG)");
  ...
  Host_sum_Widom_HGGG(NumberOfTrials, SystemComponents.Beta, OverlapCriteria, Sims.Blocksum, Sims.flag, HG_Nblock, HGGG_Nblock, energies, Trialindex, Rosen);
...
static inline double Widom_Move_Chain_PARTIAL(Components& SystemComponents, Simulations& Sims, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, int MoveType, size_t *REAL_Selected_Trial, bool *SuccessConstruction, MoveEnergy *energy, size_t FirstBeadTrial, double2 proposed_scale)
{
  ...
  std::vector<MoveEnergy>energies; std::vector<size_t>Trialindex;
  // Determine the MolID
  size_t SelectedMolID = 0;
  ...
  Random.Check(Widom.NumberWidomTrialsOrientations * 3);
  ...
  Random.Update(Widom.NumberWidomTrialsOrientations * 3);
  ...
  if(Atomsize != 0)
  {
    //Setup_threadblock(Atomsize * chainsize, &Nblock, &Nthread); // use threadsNeeded for setting up the threads and blocks
    //Collapse_Framework_Energy_OVERLAP_PARTIAL<<<Nblock*Widom.NumberWidomTrialsOrientations, Nthread, Nthread * sizeof(double)>>>(Sims.Box, Sims.d_a, Sims.New, FF, Sims.Blocksum, SelectedComponent, Atomsize, Sims.device_flag, threadsNeeded, chainsize, Nblock); checkCUDAError("Error calculating energies (PARTIAL SUM, Orientations)");
  ...
  //REZERO_VALS<<<1,1>>>(Sims.Blocksum, Nblock);

  //Setup calculation for separated HG + GG interactions//
  Collapse_Framework_Energy_OVERLAP_PARTIAL_HGGG<<<HGGG_Nblock * Widom.NumberWidomTrialsOrientations, HGGG_Nthread, HGGG_Nthread * sizeof(double)>>>(Sims.Box, Sims.d_a, Sims.New, FF, Sims.Blocksum, SelectedComponent, Atomsize, Sims.device_flag, threadsNeeded, chainsize, HGGG_Nblock, HG_Nblock); checkCUDAError("Error calculating energies (PARTIAL SUM HGGG Orientation)");

  cudaMemcpy(Sims.flag, Sims.device_flag, Widom.NumberWidomTrialsOrientations*sizeof(bool), cudaMemcpyDeviceToHost);

  Host_sum_Widom_HGGG(Widom.NumberWidomTrialsOrientations, SystemComponents.Beta, OverlapCriteria, Sims.Blocksum, Sims.flag, HG_Nblock, HGGG_Nblock, energies, Trialindex, Rosen);
