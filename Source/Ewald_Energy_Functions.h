...
__global__ void Initialize_EwaldVector_Reinsertion(Boxsize Box, int3 kmax, double* tempx, double* tempy, double* tempz, Atoms* d_a, Atoms Old, size_t Oldsize, size_t Newsize, size_t realpos, size_t numberOfAtoms, size_t SelectedComponent)
{
  ...
  for(size_t i = Oldsize; i < Oldsize + Newsize; i++) //chainsize here is the total size of the molecule for translation/rotation
  {
    Old.pos[i].x     = tempx[i - Oldsize];
    Old.pos[i].y     = tempy[i - Oldsize];
    Old.pos[i].z     = tempz[i - Oldsize];
...
__global__ void Initialize_EwaldVector_IdentitySwap(Boxsize Box, int3 kmax, double* tempx, double* tempy, double* tempz, Atoms* d_a, Atoms Old, size_t Oldsize, size_t Newsize, size_t realpos, size_t numberOfAtoms, size_t OLDComponent, size_t NEWComponent)
{
  for(size_t i = 0; i < Oldsize; i++)
  {
...
double GPU_EwaldDifference_IdentitySwap(Boxsize& Box, Atoms*& d_a, Atoms& Old, double* tempx, double* tempy, double* tempz, ForceField& FF, double* Blocksum, Components& SystemComponents, size_t OLDComponent, size_t NEWComponent, size_t UpdateLocation)
{
  ...
  // Construct exp(ik.r) for atoms and k-vectors kx, ky, kz = 0, 1 explicitly
  Initialize_EwaldVector_IdentitySwap<<<1,1>>>(Box, Box.kmax, tempx, tempy, tempz, d_a, Old, Oldsize, Newsize, UpdateLocation, numberOfAtoms, OLDComponent, NEWComponent);
