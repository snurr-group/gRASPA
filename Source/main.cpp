...
int main(void)
{
  ...
  for(size_t i = 0; i < NumberOfSimulations; i++)
  {
    ...
    {
      Read_DNN_Model(Comp_for_DNN_Model[i]);
    }
  }
    ...
    if(SystemComponents[a].UseDNNforHostGuest)
    {
      for(size_t x = 0; x < Comp_for_DNN_Model[a].DNNModel.size(); x++)
      {
        ...
        for(size_t y = 0; y < SystemComponents[a].Moleculesize[1]; y++)
        {
          ...
          printf("Atom %zu, Consider? %d, Temp: %d\n", y, SystemComponents[a].ConsiderThisAdsorbateAtom[y], TempComponents.ConsiderThisAdsorbateAtom_HOST[y]);
        }
      }
