...
double Framework_energy_CPU(Boxsize Box, Atoms* Host_System, Atoms* System, ForceField FF, Components SystemComponents)
{
  ...
  size_t selectedMol = SystemComponents.NumberOfMolecule_for_Component[1] - 1;
          ...
          for(size_t j=0; j<Componentj.size; j++)
          {
            ...
            if(!((MoleculeID == MoleculeIDB) &&(compi == compj)))
            {
              ...
              if(rr_dot < FF.CutOffVDW)
              {
                ...
                //DEBUG//
                if(MoleculeID == selectedMol) 
                {
                  if(i%Component.Molsize == 0 && j%Componentj.Molsize == 0)//First Bead//
                  {
                    FirstBeadE += result[0]; FBPairs ++;
                    textrestartFile << i << " " << j << " " << result[0] << " " << FirstBeadE << "\n";
                  }
                  else
                  { ChainE += result[0]; CPairs++;}
                }
              }
              if (!FF.noCharges && rr_dot < FF.CutOffCoul)
              {
                ...
                if(MoleculeID == selectedMol)
                {
                  if(i%Component.Molsize == 0 && j%Componentj.Molsize == 0)//First Bead//
                  {
                    FirstBeadE += resultCoul[0]; FBPairs++;
                    textrestartFile << i << " " << j << " " << resultCoul[0] << " " << FirstBeadE << "\n";
                  }
                  else
                  { ChainE += resultCoul[0]; CPairs++;}
                }
              }
            }
          }
...
__global__ void Collapse_Framework_Energy_OVERLAP_PARTIAL(Boxsize Box, Atoms* System, Atoms NewMol, ForceField FF, double* Blocksum, size_t ComponentID, size_t totalAtoms, bool* flag, size_t totalthreads, size_t chainsize, size_t NblockForTrial)
{
  ...
  if(!Blockflag)
  {
    ...
    if(cache_id == 0) {Blocksum[blockIdx.x] = sdata[0]; /*if(blockIdx.x == 21) printf("BlockID: %lu, data: %.5f\n", blockIdx.x, sdata[0]);*/}
  }
}
...
__global__ void Collapse_Framework_Energy_OVERLAP_PARTIAL_HGGG(Boxsize Box, Atoms* System, Atoms NewMol, ForceField FF, double* Blocksum, size_t ComponentID, size_t totalAtoms, bool* flag, size_t totalthreads, size_t chainsize, size_t NblockForTrial, size_t HG_Nblock)
{
