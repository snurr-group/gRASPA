...
__device__ void RotationAroundAxis(Atoms Mol,size_t i,double theta, int Axis)
{
  ...
  w=Mol.pos[i].x*rot[0*3+0]+Mol.pos[i].y*rot[0*3+1]+Mol.pos[i].z*rot[0*3+2];
  s=Mol.pos[i].x*rot[1*3+0]+Mol.pos[i].y*rot[1*3+1]+Mol.pos[i].z*rot[1*3+2];
  c=Mol.pos[i].x*rot[2*3+0]+Mol.pos[i].y*rot[2*3+1]+Mol.pos[i].z*rot[2*3+2];
  Mol.pos[i].x=w;
  Mol.pos[i].y=s;
  Mol.pos[i].z=c;
}

__global__ void get_new_position(Simulations& Sim, ForceField FF, size_t start_position, size_t SelectedComponent, double3* RANDOM, size_t index, int MoveType)
{
  ...
  switch(MoveType)
  {
    ...
    {
      Sim.New.pos[i] = pos + Sim.MaxTranslation * 2.0 * (RANDOM[index] - 0.5);
    ...
    case ROTATION://ROTATION:
    {
      ...
      const double3 Angle = Sim.MaxRotation * 2.0 * (RANDOM[index] - 0.5);
