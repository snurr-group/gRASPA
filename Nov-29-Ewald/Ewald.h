...
void Calculate_Exclusion_Energy_Rigid(Boxsize& Box, Atoms* System, ForceField FF, Components& SystemComponents)
{
  for(size_t i = 0; i < SystemComponents.Total_Components; i++)
  {
    ...
    {
      IntraE = Calculate_Intra_Molecule_Exclusion(Box, System, FF.Alpha, FF.Prefactor, SystemComponents, i);
      SelfE  = Calculate_Self_Exclusion(Box, System, FF.Alpha, FF.Prefactor, SystemComponents, i);
    }
