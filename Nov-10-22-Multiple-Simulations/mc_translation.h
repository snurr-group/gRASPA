...
__global__ void test_Sims(Simulations Sims, size_t start_position, size_t SelectedComponent, double* random, size_t offset)
{
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  size_t random_i = offset;
  size_t real_pos = start_position + i;
  Atoms Temp = Sims.d_a[SelectedComponent];
  double x         = Temp.x[real_pos];
  double y         = Temp.y[real_pos];
  double z         = Temp.z[real_pos];
  double scale     = Temp.scale[real_pos];
  double charge    = Temp.charge[real_pos];
  double scaleCoul = Temp.scaleCoul[real_pos];
  size_t Type      = Temp.Type[real_pos];
  size_t MolID     = Temp.MolID[real_pos];

  //Atoms Temp = Sims.Old;
  //Temp.x[i] = x; Temp.y[i] = y; Temp.z[i] = z;
  //Sims.New.x[i] = x + Sims.MaxTranslation.x * 2.0 * (random[random_i]   - 0.5);
  //Sims.New.y[i] = y + Sims.MaxTranslation.y * 2.0 * (random[random_i+1] - 0.5);
  //Sims.New.z[i] = z + Sims.MaxTranslation.z * 2.0 * (random[random_i+2] - 0.5);
  //Sims.deviceNew[0].size = 10;
  printf("TEST xyz: %.10f %.10f %.10f\n", x, y, z);
}
...
static inline double Translation_Move(Boxsize& Box, Components& SystemComponents, Simulations& Sims, WidomStruct& Widom, ForceField& FF, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent)
{
  ...
  test_Sims<<<1,1>>>(Sims, start_position, SelectedComponent, Random.device_random, Random.offset);
  ...
  //cudaMemset(Sims.device_flag, false, sizeof(bool)); //Zhao's note: only need the first value for the flag//
