...
void read_simulation_input(bool *UseGPUReduction, bool *Useflag, bool *noCharges, int *InitializationCycles, int *EquilibrationCycles, int *ProductionCycles, size_t *Widom_Trial, size_t *Widom_Orientation, size_t *NumberOfBlocks, double *Pressure, double *Temperature, bool *DualPrecision, size_t *AllocateSize)
{
...
void read_FFParams_from_input(ForceField& FF, Boxsize& Box)
{
  ...
  printf("tol: %.5f, tol1: %.5f, ALpha is %.5f, kmax: %d %d %d, ReciprocalCutOff: %.5f\n", tol, tol1, FF.Alpha, Box.kmax.x, Box.kmax.y, Box.kmax.z, Box.ReciprocalCutOff);
}
...
void OverWriteFrameworkCharge(Boxsize& Box, Atoms& Framework, PseudoAtomDefinitions& PseudoAtom, std::string& poscarfile)
{
  std::vector<std::string> termsScannedLined{};
  termsScannedLined = split(poscarfile, '.');
  std::string frameworkName = termsScannedLined[0];
  std::string ChargeFile = frameworkName + ".charge";
  printf("CHARGE FILE IS: %s\n", ChargeFile.c_str());
  std::ifstream simfile(ChargeFile);
  std::string str;
  size_t i = 0;
  while (std::getline(simfile, str))
  {
      ...
      double Charge = std::stod(termsScannedLined[5]);
      Framework.charge[i] = Charge;
      //printf("i: %zu, charge: %.5f\n", i, Charge);
      if(i >= Framework.size) throw std::runtime_error("Lines in Charge file does not match POSCAR! Abort.");
      i++;
  }
}
...
void POSCARParser(Boxsize& Box, Atoms& Framework, PseudoAtomDefinitions& PseudoAtom)
{
  ...
  if((fabs(Cell[3]) + fabs(Cell[6]) + fabs(Cell[7])) > 1e-10) Box.Cubic = false;
  ...
  if(UseChargesFromCIFFile) OverWriteFrameworkCharge(Box, Framework, PseudoAtom, poscarfile);
}
