...
static inline double Widom_Move_FirstBead(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, bool Retrace, double &StoredR, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, bool DualPrecision)
{
  ...
  //bool flag[NewMol.size]; 
  //bool *device_flag;
  
  //cudaMallocHost(&device_flag, NewMol.size * sizeof(bool)); checkCUDAError("error Allocating on Host");
  //cudaMalloc(&device_flag, NewMol.size * sizeof(bool)); checkCUDAError("error Allocating on Host");
  ...
  {
    cudaMemset(Widom.device_flag, false, NewMol.size*sizeof(bool));
    if(DualPrecision)
    {
      Collapse_Framework_Energy_OVERLAP_FLOAT<<<Nblock, Nthread>>>(Box, d_a, NewMol, FF, Widom.WidomFirstBeadResult, SelectedComponent, arraysize, Widom.device_flag, threadsNeeded,1, Widom.floatResult); checkCUDAError("Error calculating energies (float)");
    }
    else
    {
      Collapse_Framework_Energy_OVERLAP<<<Nblock, Nthread>>>(Box, d_a, NewMol, FF, Widom.WidomFirstBeadResult, SelectedComponent, arraysize, Widom.device_flag, threadsNeeded,1); checkCUDAError("Error calculating energies");
    }
    cudaMemcpy(Widom.flag, Widom.device_flag, NewMol.size*sizeof(bool), cudaMemcpyDeviceToHost);
  }
  ///////////////////////
  //     Reduction     //
  ///////////////////////
  if(Widom.UseGPUReduction) //Perform Reduction on the GPU//
  {
    ...
    {
      GPU_Sum_Widom(NumberOfTrials, SystemComponents.Beta, OverlapCriteria, Widom.floatResult, Widom.flag, arraysize, energies, Trialindex, Rosen);
    }
    else
    {
      GPU_Sum_Widom(NumberOfTrials, SystemComponents.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, Widom.flag, arraysize, energies, Trialindex, Rosen);
    }
  }
  else //Perform Reduction on the CPU//
  {
    if(DualPrecision && NumberOfTrials > 1){ //If there is only 1 trial position, there is no need to do Dual-Precision CBMC//
      Host_sum_Widom(NumberOfTrials, SystemComponents.Beta, static_cast<float>(OverlapCriteria), Widom.floatResult, Widom.flag, arraysize, energies, Trialindex, Rosen);}
    else{
      Host_sum_Widom(NumberOfTrials, SystemComponents.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, Widom.flag, arraysize, energies, Trialindex, Rosen);}
  }
...
static inline double Widom_Move_Chain(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, size_t FirstBeadTrial, bool DualPrecision)
{
  ...
  //bool flag[Widom.NumberWidomTrialsOrientations]; 
  //bool* device_flag;
  //cudaMalloc(&device_flag, Widom.NumberWidomTrialsOrientations * sizeof(bool));
  // Determine the MolID
  size_t SelectedMolID = 0;
  ...
  //printf("Chain: Selected MolID is %zu\n", SelectedMolID);
  ...
  {
    cudaMemset(Widom.device_flag, false, Widom.NumberWidomTrialsOrientations*sizeof(bool));
    if(DualPrecision)
    {
      Collapse_Framework_Energy_OVERLAP_FLOAT<<<Nblock, Nthread>>>(Box, d_a, NewMol, FF, Widom.WidomFirstBeadResult, SelectedComponent, arraysize, Widom.device_flag, threadsNeeded, chainsize, Widom.floatResult); checkCUDAError("Error calculating energies (float)");
    }
    else
    {
      Collapse_Framework_Energy_OVERLAP<<<Nblock, Nthread>>>(Box, d_a, NewMol, FF, Widom.WidomFirstBeadResult, SelectedComponent, arraysize, Widom.device_flag, threadsNeeded, chainsize); checkCUDAError("Error calculating energies");
    }
    cudaMemcpy(Widom.flag, Widom.device_flag, Widom.NumberWidomTrialsOrientations*sizeof(bool), cudaMemcpyDeviceToHost);
  }
  ...
  {
    if(DualPrecision){
      GPU_Sum_Widom(Widom.NumberWidomTrialsOrientations, SystemComponents.Beta, OverlapCriteria, Widom.floatResult, Widom.flag, arraysize*chainsize, energies, Trialindex, Rosen);}
    else{
      GPU_Sum_Widom(Widom.NumberWidomTrialsOrientations, SystemComponents.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, Widom.flag, arraysize*chainsize, energies, Trialindex, Rosen);}
  }
  else //Sum on the CPU//
  {
    if(DualPrecision){
      Host_sum_Widom(Widom.NumberWidomTrialsOrientations, SystemComponents.Beta, static_cast<float>(OverlapCriteria), Widom.floatResult, Widom.flag, arraysize*chainsize, energies, Trialindex, Rosen);}
    else{
      Host_sum_Widom(Widom.NumberWidomTrialsOrientations, SystemComponents.Beta, OverlapCriteria, Widom.WidomFirstBeadResult, Widom.flag, arraysize*chainsize, energies, Trialindex, Rosen);}
  }
...
static inline double Widom_Move_FirstBead_PARTIAL(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, bool Retrace, double &StoredR, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, bool DualPrecision)
{
  ...
  cudaMemset(Widom.device_flag, false, NewMol.size*sizeof(bool));
  Collapse_Framework_Energy_OVERLAP_PARTIAL<<<Nblock*NumberOfTrials, Nthread, Nthread * sizeof(double)>>>(Box, d_a, NewMol, FF, Widom.Blocksum, SelectedComponent, Atomsize, Widom.device_flag, threadsNeeded,1, Nblock); checkCUDAError("Error calculating energies (PARTIAL SUM)");
  cudaMemcpy(Widom.flag, Widom.device_flag, NewMol.size*sizeof(bool), cudaMemcpyDeviceToHost);

  ///////////////////////
  //     Reduction     //
  ///////////////////////
  Host_sum_Widom(NumberOfTrials, SystemComponents.Beta, OverlapCriteria, Widom.Blocksum, Widom.flag, Nblock, energies, Trialindex, Rosen);
...
static inline double Widom_Move_Chain_PARTIAL(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool Insertion, bool Reinsertion, size_t *REAL_Selected_Trial, bool *SuccessConstruction, double *energy, size_t FirstBeadTrial, bool DualPrecision)
{
  ...
  cudaMemset(Widom.device_flag, false, Widom.NumberWidomTrialsOrientations*sizeof(bool));
  Collapse_Framework_Energy_OVERLAP_PARTIAL<<<Nblock*Widom.NumberWidomTrialsOrientations, Nthread, Nthread * sizeof(double)>>>(Box, d_a, NewMol, FF, Widom.Blocksum, SelectedComponent, Atomsize, Widom.device_flag, threadsNeeded, chainsize, Nblock); checkCUDAError("Error calculating energies (PARTIAL SUM, Orientations)");
  ...
  //    Reduction   //
  ////////////////////
  Host_sum_Widom(Widom.NumberWidomTrialsOrientations, SystemComponents.Beta, OverlapCriteria, Widom.Blocksum, Widom.flag, Nblock, energies, Trialindex, Rosen);
