...
__global__ void Energy_difference_PARTIAL_FLAG(Boxsize Box, Atoms* System, Atoms Mol, Atoms NewMol, ForceField FF, double* BlockEnergy, size_t ComponentID, size_t totalAtoms, size_t chainsize, bool* flag) // Consider to change this for polyatomic
{
  ///////////////////////////////////////////////////////
  //All variables passed here should be device pointers//
  ///////////////////////////////////////////////////////
...
__global__ void Energy_difference_PARTIAL_FLAG_HGGG(Boxsize Box, Atoms* System, Atoms Mol, Atoms NewMol, ForceField FF, double* BlockEnergy, size_t ComponentID, size_t totalAtoms, size_t chainsize, bool* flag, size_t HG_Nblock, size_t GG_Nblock)
{
  ///////////////////////////////////////////////////////
  //All variables passed here should be device pointers//
  ///////////////////////////////////////////////////////
...
__global__ void get_new_position(Atoms* d_a, Atoms Mol, Atoms NewMol, ForceField FF, size_t start_position, size_t SelectedComponent, double* random, size_t offset, double3 Max, bool* device_flag, int MoveType)
{
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  size_t random_i = offset; // all atoms in the molecule are moving in the same direction, so no i in this variable
  size_t real_pos = start_position + i;
  ...
  switch(MoveType)
  {
    ...
    case 1://ROTATION:
    {
      NewMol.x[i] = Mol.x[i] - d_a[SelectedComponent].x[start_position];
      NewMol.y[i] = Mol.y[i] - d_a[SelectedComponent].y[start_position];
      NewMol.z[i] = Mol.z[i] - d_a[SelectedComponent].z[start_position];
      double XAngle = Max.x * 2.0 * (random[random_i] - 0.5);
      double YAngle = Max.y * 2.0 * (random[random_i+1] - 0.5);
      double ZAngle = Max.z * 2.0 * (random[random_i+2] - 0.5);
      RotationAroundXAxis(NewMol, i, XAngle);
      RotationAroundYAxis(NewMol, i, YAngle);
      RotationAroundZAxis(NewMol, i, ZAngle);
      NewMol.x[i] += d_a[SelectedComponent].x[start_position];
      NewMol.y[i] += d_a[SelectedComponent].y[start_position];
      NewMol.z[i] += d_a[SelectedComponent].z[start_position];
      break;
    }
  }
  Mol.scale[i] = scale; Mol.charge[i] = charge; Mol.scaleCoul[i] = scaleCoul; Mol.Type[i] = Type; Mol.MolID[i] = MolID;
...
__global__ void get_new_rotation_position(Atoms* d_a, Atoms Mol, Atoms NewMol, ForceField FF, size_t start_position, size_t SelectedComponent, double* random, size_t offset, double3 MaxRotation, bool* device_flag)
{
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  size_t random_i = offset; // all atoms in the molecule are moving in the same direction, so no i in this variable
  size_t real_pos = start_position + i;
...
static inline MoveEnergy SingleMove(Components& SystemComponents, Simulations& Sims, WidomStruct& Widom, ForceField& FF, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent, int MoveType)
{
  ...
  get_new_position<<<1, chainsize>>>(Sims.d_a, Sims.Old, Sims.New, FF, start_position, SelectedComponent, Random.device_random, Random.offset, Max, Sims.device_flag, MoveType); 
  ...
  Energy_difference_PARTIAL_FLAG_HGGG<<<HGGG_Nblock, HGGG_Nthread, HGGG_Nthread * sizeof(double)>>>(Sims.Box, Sims.d_a, Sims.Old, Sims.New, FF, Sims.Blocksum, SelectedComponent, Atomsize, chainsize, Sims.device_flag, HG_Nblock, GG_Nblock);
  ...
  {
   SystemComponents.Tmmc[SelectedComponent].Update(1.0, NMol, TRANSLATION_ROTATION);
   tot.zero();
   return tot;
  }
