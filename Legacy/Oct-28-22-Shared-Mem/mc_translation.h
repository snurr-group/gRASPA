double Translation_Move(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, double* y, double* dUdlambda, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent);
...
//Rotation Move//
double Rotation_Move(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, double* y, double* dUdlambda, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent);
...
static inline double Translation_Move(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, double* y, double* dUdlambda, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent)
{
  double tot = 0.0;
  //double result = 0.0;
  ...
  size_t Nthread=0; size_t Nblock=0;  Setup_threadblock(Atomsize, &Nblock, &Nthread);
  double* Blocky;  cudaMalloc(&Blocky,  Nblock * sizeof(double));
  double* BlockdU; cudaMalloc(&BlockdU, Nblock * sizeof(double));
  Energy_difference_PARTIAL<<<Nblock, Nthread, Nthread * sizeof(double2)>>>(Box, d_a, Mol, NewMol, FF, Blocky, BlockdU, SelectedComponent, Atomsize, chainsize, Nthread);
  double BlockResult[Nblock];
  cudaMemcpy(BlockResult, Blocky, Nblock * sizeof(double), cudaMemcpyDeviceToHost);
...
static inline double Rotation_Move(Boxsize& Box, Components& SystemComponents, Atoms*& System, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, double* y, double* dUdlambda, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent)
{
  double tot = 0.0;
