...
inline void Setup_System_Units_and_Box(Units& Constants, Components& SystemComponents, Boxsize& Box, Boxsize& device_Box)
{
  ...
  device_Box.kmax        = Box.kmax;
  device_Box.ReciprocalCutOff = Box.ReciprocalCutOff;
}
...
inline void Allocate_Copy_Ewald_Vector(Boxsize& device_Box, Components SystemComponents, ForceField& device_FF)
{
  //Zhao's note: This only works if the box size is not changed, eik_xy might not be useful if box size is not changed//
  size_t eikx_size     = SystemComponents.eik_x.size();
  size_t eiky_size     = SystemComponents.eik_y.size();
  size_t eikz_size     = SystemComponents.eik_z.size();
