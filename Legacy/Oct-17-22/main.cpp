...
int main(void)
{
  ...
  size_t MaxTrialsize = max(Widom.NumberWidomTrials, Widom.NumberWidomTrialsOrientations*(TemporaryComponents.Moleculesize[1]-1));
  Widom.WidomFirstBeadResult = CUDA_allocate_double_array(MaxTrialsize*(System[0].Allocate_size+System[1].Allocate_size));
  //Widom.WidomFirstBeadResult = PINNED_CUDA_allocate_double_array(MaxTrialsize*(System[0].Allocate_size+System[1].Allocate_size));
  Widom.WidomFirstBeadAllocatesize = MaxTrialsize*(System[0].Allocate_size+System[1].Allocate_size);
  ...
  cudaMemcpy(device_System, Sims[0].d_a, 2*sizeof(Atoms), cudaMemcpyDeviceToHost);
  double sys_energy = Framework_energy_CPU(Box, System, device_System, FF, SystemComponents[0]);
  ...
  one_thread_GPU_test<<<1,1>>>(device_Box, Sims[0].d_a, device_FF, device_xxx); 
  ...
  //Perform Moves//
  start = omp_get_wtime(); 
  sum = cuSoA(Cycles, SystemComponents[0], device_Box, Sims[0].d_a, device_Mol, device_NewMol, device_FF, device_y, device_dUdlambda, Random, Widom, Constants, sys_energy, DualPrecision);
  end = omp_get_wtime();
  ...
  for(size_t i = 0; i < NumberOfSimulations; i++)
  {
    ...
    xxx[0] = 0.0; one_thread_GPU_test<<<1,1>>>(device_Box, Sims[i].d_a, device_FF, device_xxx);
    cudaMemcpy(xxx, device_xxx, sizeof(double), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
