...
static inline double CBCF_LambdaChange(Boxsize& Box, Components& SystemComponents, Simulations& Sims, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, double2 oldScale, double2 newScale, size_t& start_position, int MoveType, bool& SuccessConstruction)
{
  double tot = 0.0;
  //double result = 0.0;
  size_t Atomsize = 0;
  ...
  Energy_difference_LambdaChange<<<Nblock, Nthread, Nthread * sizeof(double)>>>(Box, Sims.d_a, Sims.Old, FF, Sims.Blocksum, SelectedComponent, Atomsize, chainsize, Nthread, Sims.device_flag, newScale);
  ...
  if(!Sims.flag[0])
  {
    ...
    SystemComponents.tempdeltaVDWReal += tot;
    // Calculate Ewald //
    if(!FF.noCharges)
    {
      //Zhao's note: since we changed it from using translation/rotation functions to its own, this needs to be changed as well//
      EwaldE = GPU_EwaldDifference_LambdaChange(Box, Sims.d_a, Sims.Old, FF, Sims.Blocksum, SystemComponents, SelectedComponent, oldScale, newScale, MoveType);
      tot   += EwaldE;
...
double CBCFMove(Boxsize& Box, Components& SystemComponents, Simulations& Sims, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool DualPrecision);

static inline double CBCFMove(Boxsize& Box, Components& SystemComponents, Simulations& Sims, ForceField& FF, RandomNumber& Random, WidomStruct& Widom, size_t SelectedMolInComponent, size_t SelectedComponent, bool DualPrecision)
{
  ...
  if(SelectednewBin > static_cast<int>(nbin))
  {
    ...
    energy = CBCF_LambdaChange(Box, SystemComponents, Sims, FF, Random, Widom, SelectedMolInComponent, SelectedComponent, oldScale, InterScale, start_position, 5, SuccessConstruction);
    if(!SuccessConstruction) return 0.0;
    double first_step_energy = energy;
    ...
    /////////////////////////////////////////////////////
    energy += Insertion_Body(Box, SystemComponents, Sims, FF, Random, Widom, SelectedMolInComponent, SelectedComponent, DualPrecision, Rosenbluth, SuccessConstruction, SelectedTrial, preFactor, true, newScale);
    if(SuccessConstruction)
    {
      ...
      preFactor *= std::exp(-SystemComponents.Beta * first_step_energy + biasTerm);
  ...
  else if(SelectednewBin < 0) //Deletion Move//
  {
    ...
    double Rosenbluth=Widom_Move_FirstBead_PARTIAL(Box, SystemComponents, Sims, FF, Random, Widom, SelectedMolInComponent, SelectedComponent, MoveType, StoredR, &SelectedTrial, &SuccessConstruction, &energy, DualPrecision, oldScale);
    if(SystemComponents.Moleculesize[SelectedComponent] > 1 && Rosenbluth > 1e-150)
    {
      ...
      Rosenbluth*=Widom_Move_Chain_PARTIAL(Box, SystemComponents, Sims, FF, Random, Widom, SelectedMolInComponent, SelectedComponent, MoveType, &SelectedTrial, &SuccessConstruction, &energy, SelectedFirstBeadTrial, DualPrecision, oldScale); //The false is for Reinsertion//
      energy += temp_energy;
    }
    if(!SuccessConstruction) return 0.0;
    ...
    if(!FF.noCharges)
    {
      ...
      else
      {
        ...
        EwaldE = GPU_EwaldDifference_General(Box, Sims.d_a, Sims.New, Sims.Old, FF, Sims.Blocksum, SystemComponents, SelectedComponent, 6, UpdateLocation, oldScale);
      }
    ...
    {
      size_t start_position = 1;
      double second_step_energy = 0.0;
      
      SuccessConstruction = false;
      second_step_energy = CBCF_LambdaChange(Box, SystemComponents, Sims, FF, Random, Widom, SelectedMolInComponent, SelectedComponent, InterScale, newScale, start_position, 6, SuccessConstruction); //Fractional deletion, lambda change is the second step (that uses the temporary totalEik vector)
      
      //Account for the biasing terms of different lambdas//
      double biasTerm = SystemComponents.Lambda[SelectedComponent].biasFactor[newBin] - SystemComponents.Lambda[SelectedComponent].biasFactor[oldBin];
      preFactor *= std::exp(-SystemComponents.Beta * second_step_energy + biasTerm);
      if(get_random_from_zero_to_one() < preFactor * IdealRosen / Rosenbluth)
      {
        Accepted = true;
        ...
        final_energy = -first_step_energy + second_step_energy;
        SystemComponents.deltaVDWReal += SystemComponents.tempdeltaVDWReal; SystemComponents.deltaEwald += SystemComponents.tempdeltaEwald;
      }
    }
    if(!Accepted)
  ...
  else //Lambda Move//
  {
    ...
    //Account for the biasing terms of different lambdas//
    double biasTerm = SystemComponents.Lambda[SelectedComponent].biasFactor[newBin] - SystemComponents.Lambda[SelectedComponent].biasFactor[oldBin];
    if (get_random_from_zero_to_one() < std::exp(-SystemComponents.Beta * energy + biasTerm))
    {
