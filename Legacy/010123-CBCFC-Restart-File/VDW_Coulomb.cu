...
inline void CoulombReal_CPU(const ForceField FF, const double chargeA, const double chargeB, const double r, const double scaling, double* result) //energy = -q1*q2/r
{
  ...
  //double Q         = 0.01 * (1.0 - scaling); //https://pubs.acs.org/doi/10.1021/acs.jcim.1c00652, equation S13//
  //double term      = chargeA * chargeB * std::erfc(alpha * (r+Q));
  //       result[0] = prefactor * scaling * term / (r+Q);
...
__global__ void Energy_difference_LambdaChange(Boxsize Box, Atoms* System, Atoms Mol, ForceField FF, double* BlockEnergy, size_t ComponentID, size_t totalAtoms, size_t chainsize, size_t Threadsize, bool* flag, double2 newScale)
{
  ...
  if(!((MoleculeID == Mol.MolID[0]) &&(comp == ComponentID))) //ComponentID: Component ID for the molecule being translated
  {
    ...
    if (!FF.noCharges && rr_dot < FF.CutOffCoul)
    {
      ...
      CoulombReal(FF, chargeA, chargeB, r, OldscalingCoul, result);
      tempy -= result[0]; 
      CoulombReal(FF, chargeA, chargeB, r, NewscalingCoul, result);
      tempy += result[0];
    }
  }
