double Translation_Move(Boxsize& Box, Components& SystemComponents, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, double* y, double* dUdlambda, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent);
...
//Rotation Move//
double Rotation_Move(Boxsize& Box, Components& SystemComponents, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, double* y, double* dUdlambda, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent);
...
__global__ void get_new_translation_position(Atoms* d_a, Atoms Mol, Atoms NewMol, ForceField FF, size_t start_position, size_t SelectedComponent, double* random, size_t offset)
{
  ...
  //if(i == 0){NewMol.size = Molsize; Mol.size = Molsize;}
}
...
static inline double Translation_Move(Boxsize& Box, Components& SystemComponents, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, double* y, double* dUdlambda, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent)
{
  double tot = 0.0;
  //double result = 0.0;
  ...
  if((Random.offset + 3*Molsize) >= Random.randomsize) Random.offset = 0;
  get_new_translation_position<<<1, Molsize>>>(d_a, Mol, NewMol, FF, start_position, SelectedComponent, Random.device_random, Random.offset); Random.offset += 3*Molsize;
  ...
  Framework_energy_difference_SoA<<<Nblock, Nthread>>>(Box, d_a, Mol, NewMol, FF, y, dUdlambda,SelectedComponent, arraysize, Molsize);
  tot = GPUReduction<BLOCKSIZE>(y, arraysize);
  if (get_random_from_zero_to_one() < std::exp(-FF.Beta * tot))
  {
    //printf("tot: %.10f\n", tot);
    update_translation_position<<<1,Molsize>>>(d_a, NewMol, start_position, SelectedComponent);
...
__global__ void get_new_rotation_position(Atoms* d_a, Atoms Mol, Atoms NewMol, ForceField FF, size_t start_position, size_t SelectedComponent, double* random, size_t offset)
{
  ...
  //Update NewMol//
  //printf("i: %lu, Angle: %.10f %.10f %.10f, Old: %.10f %.10f %.10f, Dist: %.10f %.10f %.10f\n", i, XAngle, YAngle, ZAngle, Mol.x[i],Mol.y[i],Mol.z[i],  NewMol.x[i],NewMol.y[i],NewMol.z[i]);
  NewMol.x[i] += AllData.x[start_position]; NewMol.y[i] += AllData.y[start_position]; NewMol.z[i] += AllData.z[start_position];
  //printf("i: %lu, Old: %.10f %.10f %.10f, Rotated: %.10f %.10f %.10f\n", i, Mol.x[i],Mol.y[i],Mol.z[i],  NewMol.x[i],NewMol.y[i],NewMol.z[i]);
  Mol.scale[i] = scale; Mol.charge[i] = charge; Mol.scaleCoul[i] = scaleCoul; Mol.Type[i] = Type; Mol.MolID[i] = MolID;
  NewMol.scale[i] = scale; NewMol.charge[i] = charge; NewMol.scaleCoul[i] = scaleCoul; NewMol.Type[i] = Type; NewMol.MolID[i] = MolID;
  //if(i == 0){NewMol.size = Molsize; Mol.size = Molsize;}
}

static inline double Rotation_Move(Boxsize& Box, Components& SystemComponents, Atoms*& d_a, Atoms& Mol, Atoms& NewMol, ForceField& FF, double* y, double* dUdlambda, RandomNumber& Random, size_t SelectedMolInComponent, size_t SelectedComponent)
{
  double tot = 0.0;
  ...
  for(size_t ijk = 0; ijk < SystemComponents.Total_Components; ijk++)
  {
    SystemComponents.NumberOfMolecule_for_Component[ijk] * SystemComponents.Moleculesize[ijk];
  }
  //Set up Old position and New position arrays
  size_t Molsize = SystemComponents.Moleculesize[SelectedComponent]; //Get the size of the selected Molecule
  size_t Allocate_size = 1024;
  ...
  get_new_rotation_position<<<1,Molsize>>>(d_a, Mol, NewMol, FF, start_position, SelectedComponent, Random.device_random, Random.offset); Random.offset += 3*Molsize;
