...
int main(void)
{
  ...
  read_simulation_input(&Widom.UseGPUReduction, &Widom.Useflag, &FF.noCharges, &Cycles, &Widom.NumberWidomTrials, &Widom.NumberWidomTrialsOrientations, &MoveStats.NumberOfBlocks, &Box.Pressure, &Box.Temperature, &DualPrecision, &Allocate_space_Adsorbate);
  ...
  std::vector<double> sys_energy(NumberOfSimulations);
  for(size_t i = 0; i < NumberOfSimulations; i++)
  {
    if(RunSingleSim){if(i != SelectedSim) continue;}
  ...
  if(!RunSingleSim)
  {
    Multiple_Sims(Cycles, SystemComponents, device_Box, Sims, device_FF, Random, WidomArray, Constants, sys_energy);
  }
  else
  {
    for(size_t i = 0; i < NumberOfSimulations; i++)
    {
      ...
      sum[i] = Run_Simulation(Cycles, SystemComponents[i], device_Box, Sims[i], device_FF, Random, WidomArray[i], Constants, sys_energy[i], DualPrecision, CreateMolWrapper[i], false);
    }
  }
  double end = omp_get_wtime();
